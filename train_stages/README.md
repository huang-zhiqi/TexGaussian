...
```
阶段1: 训练新增模块 (5-6h / 2×3090)
    TextAdapter + GGCA + Normal Head + Rotation Head
    base UNet 冻结，仅训练 ~1.72M 新参数
    ↓ 验证：albedo 颜色正确，normal_geo/tex_loss 同步下降，PSNR 22-25
...
| 项目 | 说明 |
|------|------|
| 目标 | 训练 TextAdapter + GGCA + Normal/Rotation Heads |
| 时间 | 30 epochs，约 5-6 小时 (2×3090) |
| 学习率 | 4e-4 |
| 训练参数 | ~1.72M（占 base 的 0.58%） |
...
| Normal Head | 116K | 2-layer OctreeHead (64→64→3) |
| Rotation Head | 118K | 2-layer OctreeHead (64→64→4) |

**检查点：**
- [ ] albedo 颜色正确
- [ ] albedo_loss / material_loss 稳步下降
- [ ] normal_geo_loss 稳步下降
- [ ] normal_tex_loss 稳步下降
- [ ] PSNR 达到 22-25
...
- `albedo_loss`: 应该持续下降
- `psnr`: 应该持续上升
- `normal_geo_loss`: 应该下降
- `normal_tex_loss`: 应该下降
...
### 2. normal loss 不下降
- 检查数据集是否包含 normal maps
- 增加 `lambda_geo_normal` 和 `lambda_tex_normal`
